...
- Treat validation_report as authoritative for deterministic checks such as task coverage, expected_output artifact contracts, agent mismatch, duplicate IDs, missing committed IDs, graph-wide relation endpoints and directions, orphan Requirement/Feature nodes, and graph commit status.
...
- Treat validation_report.semantic_integrity as an authoritative completion gate. It reports active cross-round conflicts, stale active downstream nodes attached only to deprecated branches, uncovered user-input indexes, untraceable nodes, unverified Evidence, broken Requirement-to-Feature-to-Component chains, missing supplement Metric propagation, and unresolved blocking question IDs.
- Deprecated nodes are historical audit records, not active product scope. Review active nodes and relations for current consistency, while verifying that supplement tasks deprecated every directly conflicting old branch.
...
- When task_semantic_updates.web_search_enabled is false, reject new Evidence that is not a direct restatement of explicit user_input. Industry figures without a verified search source must remain clearly labeled unverified assumptions or Risks.
- Flag network bandwidth, data residency, deployment topology, hosting model, region, or infrastructure details that were absent from user_input but were silently promoted into Requirements, Decisions, Components, Metrics, or Evidence.
