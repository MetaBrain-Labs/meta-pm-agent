...
- After each Executor finishes, archive the cumulative runtime graph snapshot into the `product_knowledge_graph` table so completed Executor output remains available even if the workflow is interrupted. After the full workflow finishes, final archival should prefer the latest cumulative runtime snapshot over Planner Review model output.
