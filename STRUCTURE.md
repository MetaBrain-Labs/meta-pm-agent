...
- `product_knowledge_graph` stores the latest structured graph for each workspace: generated markdown `content`, `summary`, `nodes`, `relations`, `decisions`, `risks`, and `open_questions`. It has one current row per `workspace_id`, optional `conversation_id` / `request_form_id` provenance, and an incrementing `version`.
